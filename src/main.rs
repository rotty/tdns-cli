...
use trust_dns::{
    ...
    proto::{
        ...
        udp::UdpResponse,
        xfer::{dns_multiplexer::DnsMultiplexerSerialResponse, dns_request::DnsRequest},
    },
    ...
    O: DnsOpen,
{
    ...
    fn run(&mut self) -> impl Future<Item = (), Error = failure::Error> {
        ...
        let get_authorative = get_ns_records(self.recursor.clone(), settings.domain.clone())
            .map_err(failure::Error::from);
        ...
        I: IntoIterator<Item = rr::Name>,
    {
        future::join_all(authorative.into_iter().map(move |server_name| {
            ...
            let resolve = resolve_authorative(recursor.clone(), server_name.clone()).map(move |ip| {
